...
homicidios_ENPOL_ENVIPE.fn <- function(
      ...
      plt <- ggplot(data2plot, 
        ...
        geom_line(data = data2plot1, 
                  ...
                  size = 1,
                  show.legend = FALSE) +
        geom_text(data = data2plot2,  # Add this to display labels on columns
                  aes(x = Anio,
                      y = value2plot,
                      label = labels,
                      group = group_var),
...
# 1.1 delitos sentenciados sólo común -------------------------------------
  ...
  plt <- ggplot(data2plot, 
    ...
    theme(panel.grid.major.y = element_blank(),
          ...
          axis.text.y=element_text(family   = "Lato Full",
                                   ...
                                   margin   = margin(0, 10, 0, 0),
                                   hjust = 0),
          plot.title = element_text(color="#524F4C", 
                                    size=10,
                                    vjust = -0.5)) +
    coord_flip()+
    ggtitle("Sólo delitos del fueron común en CDMX")+
    labs(caption = "N=850"); plt

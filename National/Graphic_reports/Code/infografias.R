...
writexl::write_xlsx(x = data_subset.df, 
                    path = paste0(
                                  ...
                                  "/Output/", savePath, "/Infografias/",
                                  "Derecho a la información, a la no autoincriminacion y a una justicia pronta2.xlsx"))
...
openxlsx::write.xlsx(x = defensa_oportuna, 
                     file = paste0(
                       path2SP, "National/Graphic_reports", 
                       "/Output/", savePath, "/Infografias/",
                       "Derecho a una defensa oportua y adecuada2.xlsx"))
...
data_subset.df_1 <- master_data.df %>%
  ...
  summarise(
    ...
    proceso = mean(proceso_justo, na.rm = T)
  ) %>%

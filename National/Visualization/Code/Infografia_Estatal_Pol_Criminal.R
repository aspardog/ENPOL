...
snsp_2<- snsp %>%
  ...
  mutate(Delito=case_when(`Subtipo de delito`== "Homicidio doloso" ~ "Homicidio doloso, SESNSP",
         ...
         Entidad = case_when(Entidad == "Ciudad de M\xe9xico" ~ "Distrito Federal",
                             ...
                             T ~ Entidad)) %>% 
  group_by(Delito,Anio, Entidad) %>%
  summarize(Homicidio_doloso_SESNSP = sum(Incidencia)) %>% 
  filter(Delito != "Otro") %>% 
  ungroup() %>%
  select(- Delito)

master_data.df <- master_data.df %>%
  left_join(snsp_2, by = c("Anio_arresto" = "Anio", "Estado_arresto" = "Entidad"))
...
data_subset.df <- master_data.df %>%
  ...
  mutate(corporacion_fuero = case_when(Corporacion_grupos == "Ejército o Marina" ~ "Corporación Federal", 
    ...
    `7.7a` = case_when(P5_11_01 == 1 ~ 1,
                       P5_11_01 == 0 ~ 0,
                       T ~ NA_real_),
    `8.1a` = case_when(fuero == "Sólo federal" ~ 1,
                       fuero == "Sólo común" ~ 0,
                     T ~ NA_real_),
    `8.2a` = case_when(fuero == "Sólo federal" ~ 0,
                       fuero == "Sólo común" ~ 1,
                     T ~ NA_real_),
  ...
  select(Estado_arresto, `1a`, `2a`, `3a`, `4a`, `5.1a`, `6.1a`,`6.2a`, `7.1a`,`7.2a`,`7.3a`,`7.4a`,`7.5a`,`7.6a`,`7.7a`,
         `8.1a`,`8.2a`, `9.1a`, `9.2a`, `10.1a`, `10.2a`, `10.3a`, `10.4a`, `11.1a`, `11.2a`, `12.1a`, `12.2a`, `12.3a`,

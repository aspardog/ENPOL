...
data_subset.df <- master_data.df %>%
  ...
  mutate(corporacion_fuero = case_when(Corporacion_grupos == "Ejército o Marina" ~ "Corporación Federal", 
    ...
    `6.2a` = case_when(P5_11_12 == 1 | P5_31_12 == 1 ~ 1,
                       P5_11_12 == 0 ~ 0,
                       P5_31_12 == 0 ~ 0,
                       T ~ NA_real_)*FAC_PER, #reajustada al crear la tabla final
    ...
    `13.4a` = case_when(tiempo_sentencia >= 0 & tiempo_sentencia < 4.99 ~ 0,
                        ...
                        tiempo_sentencia >= 20  ~ 0,
                        T ~ NA_real_),
    `13.5a` = case_when(tiempo_sentencia >= 0 & tiempo_sentencia < 4.99 ~ 0,
                        tiempo_sentencia >= 4.99 & tiempo_sentencia < 9.99 ~ 0,
                        tiempo_sentencia >= 9.99 & tiempo_sentencia < 14.99 ~ 0,
                        tiempo_sentencia >= 14.99 & tiempo_sentencia < 19.99 ~ 0,
                        tiempo_sentencia >= 20  ~ 1,
                        T ~ NA_real_),
...
det_hom_nac <- data_subset.df %>% mutate(Estado = "Promedio Nacional") %>% group_by(Estado) %>% summarise(detenidos_hom = sum(`6.2a`, na.rm = T)) %>% select(detenidos_hom) %>% unlist()
det_rob_nac <- data_subset.df %>% mutate(Estado = "Promedio Nacional") %>% group_by(Estado) %>% summarise(detenidos_rob = sum(`6.4a`, na.rm = T)) %>% select(detenidos_rob) %>% unlist()

porc_hom_edo <- data_subset.df %>% rename(Estado = Estado_arresto) %>% group_by(Estado) %>% summarise(detenidos_hom = sum(`6.2a`, na.rm = T)) %>% filter(!is.na(Estado))
porc_rob_edo <- data_subset.df %>% rename(Estado = Estado_arresto) %>% group_by(Estado) %>% summarise(detenidos_rob = sum(`6.4a`, na.rm = T)) %>% filter(!is.na(Estado))
